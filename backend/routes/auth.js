...
import { register, login, getUser, verifyEmail, getPublicProfile, addPhoto, deletePhoto, reportUser } from '../controllers/authController.js';
...
router.get('/user/:userId/profile', verifyToken, getPublicProfile);
router.post('/user/photos', verifyToken, upload.single('photo'), addPhoto);
router.delete('/user/photos/:filename', verifyToken, deletePhoto);
router.post('/user/:userId/report', verifyToken, reportUser);
