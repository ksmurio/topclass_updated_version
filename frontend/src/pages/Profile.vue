<template>
    ...
    <v-container class="page-container">
      ...
      <div class="section-block mb-4">
        ...
        <div v-else class="photo-grid">
          <div
            v-for="filename in photos"
            :key="filename"
            class="photo-item"
            @click="openModal(filename)"
          >
            <img
            ...
            <v-btn
              ...
              @click.stop="deletePhoto(filename)"
            >
            ...
            <v-btn
              ...
              size="x-small"
              color="warning"
              class="photo-action-btn report-btn"
              @click.stop="reportPhoto(filename)"
            >
  ...
  <v-dialog v-model="modal" max-width="90vw">
    <v-card style="background: black; display: flex; align-items: center; justify-content: center; padding: 8px; position: relative;">
      ...
      <img
        :src="`http://localhost:3000/uploads/${modalSrc}`"
  ...
  try {
    if (isPublicView.value) {
      const res = await axios.get(
        `http://localhost:3000/api/auth/user/${route.params.userId}/profile`,
        { headers }
      );
...
const uploadPhoto = async (event) => {
  ...
  try {
    const res = await axios.post(
      'http://localhost:3000/api/auth/user/photos',
      formData,
      { headers: { ...headers, 'Content-Type': 'multipart/form-data' } }
    );
    if (res.data.success) {
      user.value.photos = res.data.photos;
      showSnack('Foto adicionada!');
    }
  } catch (e) {
...
const deletePhoto = async (filename) => {
  try {
    const res = await axios.delete(
      `http://localhost:3000/api/auth/user/photos/${filename}`,
      { headers }
    );
    if (res.data.success) {
      user.value.photos = res.data.photos;
...
const reportPhoto = async (filename) => {
  try {
    await axios.post(
      `http://localhost:3000/api/auth/user/${route.params.userId}/report`,
      { filename },
      { headers }
    );
